...
for ii in serieslength:
    for jj in range(100):
        mydf = np.expand_dims(np.loadtxt('./trend_nonlineardata/simdata_trend_nonlinear_'+str(ii)+'_sim'+str(jj+1)+'.txt'),2)
        prop_pred = np.loadtxt('./trend_nonlinearsimresults/prop_simresults_trend_nonlinear_' + str(ii) + '_sim'+str(jj+1)+'.txt')
        ftsa_pred = np.loadtxt('./trend_nonlinearsimresults/funseries_simresults_trend_nonlinear_' + str(ii) + '_sim'+str(jj+1)+'.txt').T
        fplsr_pred = np.loadtxt('./trend_nonlinearsimresults/fplsr_simresults_trend_nonlinear_' + str(ii) + '_sim'+str(jj+1)+'.txt').T
        dfpca_pred = np.loadtxt('./trend_nonlinearsimresults/dfpca_simresults_trend_nonlinear_' + str(ii) + '_sim'+str(jj+1)+'.txt')
