...
for ii in serieslength:
    for jj in range(100):
        ...
        prop_pred = np.loadtxt('./linearsimresults/prop_simresults_linear_' + str(ii) + '_sim'+str(jj+1)+'.txt')
        ftsa_pred = np.loadtxt('./linearsimresults/funseries_simresults_linear_' + str(ii) + '_sim'+str(jj+1)+'.txt').T
        fplsr_pred = np.loadtxt('./linearsimresults/fplsr_simresults_linear_' + str(ii) + '_sim'+str(jj+1)+'.txt').T
        dfpca_pred = np.loadtxt('./linearsimresults/dfpca_simresults_linear_' + str(ii) + '_sim'+str(jj+1)+'.txt')
