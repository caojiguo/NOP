...
for ii in serieslength:
    ...
    for jj in range(100):
        ...
        mymod, trainingloss = fitting_wrap_noday(model = mymod, train_x = trainset, train_y = trainset,
                                            weightingvec= [1,1,1e-6],optimizer=optimizer,numofepochs=25)
        ...
        np.savetxt('./trend_nonlinearsimresults/prop_simresults_trend_nonlinear_' + str(ii) + '_sim'+str(jj+1)+'.txt' ,onesteppred)
    lengthindex += 1 
